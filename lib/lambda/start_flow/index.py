...
def lambda_handler(event, context):
        ...
        while time.time() - start_time < max_time:
            try:
                ...
                if current_state == desired_state:
                    ...
                    print("Flow is already active or starting. No action needed.")
                    return current_state
    ...
    try:
        ...
        if initial_state == 'STANDBY':
            final_state = wait_for_flow_state('ACTIVE')
            action = "Started"
        elif initial_state in ['STARTING', 'ACTIVE']:
            print("Flow is already active or starting. No action needed.")
            final_state = initial_state
            action = "No action"
        else:
            print(f"Flow is in an unexpected state: {initial_state}. Attempting to start...")
            final_state = wait_for_flow_state('ACTIVE')
            action = "Started"
